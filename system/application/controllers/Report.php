...
class Report extends controller
{
	   ...
	   '<li><a href="Report/ContactsCsv/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;Download All</span></a></li>':'';
		...
		foreach ($emp_list['data'] as $item){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= '<li>'.anchor("Report/sendSms/".$item['number']."/contact", ' <span title="Click to send SMS" class="glyphicon glyphicon-comment">&nbsp;Send SMS</span>','class="clickToSMS" data-toggle="modal" data-target="#modal-empl"').'</li>';	
				$act .= '</ul></div>';
				$arrs['action'] = $act;
			}
			foreach($keys as $k){
		...
		if($this->input->post('update_system')){
			...
			foreach($to as $to){
				if($this->session->userdata('dnd_status')!=0){
					$reply = sms_send($to,$_POST['sms_content'],$senderid);
	...
	function Send_SMS(){
		...
		$this->load->helper('mcube_helper');
		if($this->session->userdata('dnd_status')!=0){
			$reply=sms_send($_POST['to'],$_POST['sms_content'],$senderid);
			$fp = fopen("smslog.txt","a");fwrite($fp,"\n".'['.date('Y-m-d H:i:s').'] : '.'SMS:'.$reply);fclose($fp);
			$dnd_status=0;
		}else{
	...
	function mTracker(){
		...
		$ofset = ($this->uri->segment(3)!=null)?$this->uri->segment(3):0;
		$limit = '30';
		$data['itemlist'] = $this->reportmodel->mTrackerList($bid,$ofset,$limit,$url,$u3);
		$this->pagination->initialize(array(
						 'base_url'=>site_url($this->uri->segment(1).'/'.$u3).'/'
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit		
		...
		$links = array();
		$links[] = ($roleDetail['modules']['51']['opt_delete']) ?'<li><a  class="blkDeletecall" href="Report/mtBulkDel" ><span title="Bulk Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
		$links[] = '<li><a href="#" class="blkemail" rel="calls"><span title="Bulk Mail" class="glyphicon glyphicon-envelope">&nbsp;Email</span></a></li>';
		$links[] = '<li><a href="Report/blksms" class="blkSMs" data-toggle="modal" data-target="#modal-blksms" rel="calltrack"><span title="Bulk SMS" class="glyphicon glyphicon-comment">&nbsp;SMS</span></a></li>';
		$links[] = '<li  class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search">&nbsp;Search</span></a></li>';
		//~ $links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-advsearch" data-toggle="modal" data-target="#modal-advsearch" ><span title="Search" class="glyphicon glyphicon-zoom-in">&nbsp;Advance Search</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = ($roleDetail['modules']['51']['opt_download']!=0) ? '<li><a href="Report/Bulk_down/'.$csv_link1.'/'.$csv_link.'" class="blk_calls" data-toggle="modal" data-target="#modal-pop"><span title="Download" class="glyphicon glyphicon-arrow-down">&nbsp;Download Select</span></a></li>':'';
		$links[] = ($roleDetail['modules']['51']['opt_download']!=0) ? '<li><a href="Report/incomingcalls_csv/'.$csv_link1.'/'.$csv_link.'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;Download All</span></a></li>':'';
	...
	function mtedit($id){
		...
		if($this->input->post('update_system')){
			$res=$this->reportmodel->update_mtdetails($id,$bid);
			if($res == 1){
				$this->session->set_flashdata('msgt', 'error');
				$this->session->set_flashdata('msg', "You donot have enough usage of conversion of lead, please contact your account manager");
			}elseif($res == 2){
		...
		array_push($formFields,$cf);
		$data['form'] = array(
		            'form_attr'=>array('action'=>'Report/mtedit/'.$id,'name'=>'editreport','id'=>'editreport','enctype'=>"multipart/form-data"),
	...
	function mtDelete($id){
		...
		$this->reportmodel->mtDeleteCall($id,$bid);
		return 1;
	}
	function mtBulkDel(){
		$rst = $this->reportmodel->mtBulkDel($_POST['callid']);
		echo "1";	
	}
