	...
	function Businesslist(){
		...
		foreach ($emp_list['data'] as $item){
			...
			$rec[] = array(
				...
				.((isset($access1) && $access1 == '1')?('<a href="Masteradmin/sensorlist/'.$item['bid'].'"><i class="fa fa-bullseye" title="List Sensor"></i></a>'):'')
	...
	function unconfirm_list_business($bid){
		...
		foreach ($emp_list['data'] as $item){
			$rec[] = array(
				...
				,$item['empemail']
				,$item['empnumber']
				,'<a href=Masteradmin/delete_unconfirmEmployee/'.$item['eid'].'/'.$bid.'><span title="Delete" class="glyphicon glyphicon-trash"></span></a>'.'<a href="Masteradmin/confirmEmp/'.$item['eid'].'/'.$bid.'" class="confirmEmp" rel="'.$item['eid'].'"><img src="system/application/img/icons/disable.gif"/></a>'
			);
		}
		$data['itemlist']['rec'] = $rec;
		$data['itemlist']['count'] = $emp_list['count'];
...
function listlanding($bid){
	...
	function sensorlist($bid){
	$ofset = ($this->uri->segment(4)!=null)?$this->uri->segment(4):0;

		$limit = '30';
		$emp_list=$this->mastermodel->getlocationlist($ofset,$limit,$bid);
		$rec = array();
		$header=array('#');
		$i=$ofset+1;$j=0;
		if($emp_list['count']>0)
		...
		$data['links']= $links;	
		$this->pagination->initialize(array(
						 'base_url'=>site_url('Masteradmin/sensorlist/')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit	
						,'uri_segment'=>3					
				));		
