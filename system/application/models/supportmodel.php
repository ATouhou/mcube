		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($roleDetail['role']['accessrecords']=='0') ? 
					...
					:'<span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span>')
					:"";
	...
	function editSupportTkt($id,$bid){
		...
		if($itemDetail['tkt_status'] != $_POST['tkt_status']){
			...
			if($smschk['sms'] == 1 && $smschk['smscontent'] != ''){
				...
				if($smsbal > 0){
					...
					$sms = sms_send($this->input->post('number'),$remsg,$senderid);
		...
		if(isset($_POST['alert_type']) && ($this->input->post('alert_type') =='3' || $this->input->post('alert_type') =='2')){
			...
			if($smsbal > 0){
				...
				$this->load->helper('mcube_helper');
				$senderid = $this->configmodel->busSenderId($bid);
				$sms = sms_send($sql[0]['empnumber'],$message,$senderid);
				//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= '<li><a href="'.base_url().'support/undelSupportTkt/'.$r['tktid'].'"><span title="Delete" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			$i++;
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							...
							<ul class="dropdown-menu" style="text-align:left;">';
				if($type == 'del'){
					$act .= '<li><a href="'.base_url().'support/delSupportGrp/'.$rec['gid'].'/1"><span title="Delete" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>';
				}else{
