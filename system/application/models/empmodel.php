	...
	function add_emp(){
		if($this->email_employee()){
			...
			for($i=0;$i<sizeof($arr);$i++){
				if(!in_array($arr[$i],array("update_system","username","password","cpassword","reportto","recording"))){
	...
	function getDeletedEmplist($bid,$ofset='0',$limit='20'){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= '<li><a href="'.base_url().'Employee/UnDelete_Employee/'.$r['eid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			$i++;
