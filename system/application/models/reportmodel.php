	...
	function getReportlist($bid,$ofset='0',$limit='20',$type='a',$tab)
	{
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= '<li><a href="Report/activerecords/'.$r['callid'].'" class="btn-list" data-toggle="modal" data-target="#modal-list"><span class="glyphicon glyphicon-file"  title="List Employee Group">&nbsp;GroupDetails</span></a></li>';
			    $act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='' && file_exists('sounds/'.$r['filename'])) ? '<li><a target="_blank" href="'.site_url('sounds/'.$r['filename']).'"><span title="Sound" class="glyphicon glyphicon-volume-up">&nbsp;Recording</span></a></li>' :'<li><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></li>') : "";
		...
		foreach($rst as $rec){
			...
			if($opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'Report/UnDel/'.$r['callid'].'"><span title="Delete" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>':'';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
			...
			foreach($rst as $rec){
				...
				if($opt_add || $opt_view || $opt_delete){
					...
					$act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='' && file_exists('sounds/'.$r['filename']))
						...
						:'<li><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></li>'):"";
	...
	function getClickToCalls($bid,$ofset='0',$limit='20'){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= '<li><a href="Report/c2cActive/'.$rec['callid'].'"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-user"  title="List Employee Group">&nbsp;ListEmployee</span></a></li>';
				$act .= ($roleDetail['role']['accessrecords']=='0') ? (($rec['filename']!='' && file_exists('sounds/'.$rec['filename']))
					?'<li><a target="_blank" href="'.site_url('sounds/'.$rec['filename']).'"><span title="Sound" class="glyphicon glyphicon-volume-up"></span></a></li>'
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'Report/UnDelC2C/'.$r['callid'].'"><span title="Delete" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>':'';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
...
function followupdownload($bid){
	...
	function sendF($call_id,$bid,$mod,$type){
		...
		switch($case){
			case '1':
				if($_POST['smsBal']>0){
					$reply=sms_send($empDetail['empnumber'],$message,$senderid);
	...
	function trackd(){
		...
		$sql = $this->db->query("SELECT COALESCE(COUNT(callid), 0) AS day 
							...
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 DAY)) $q " ) ;	
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
		}else{
	...
	function trackw(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS week 
							FROM ".$bid."_callhistory h 
							LEFT JOIN ".$bid."_groups d ON d.gid = h.gid 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 WEEK)) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->week;
		}else{
	...
	function trackm(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_callhistory h 
							LEFT JOIN ".$bid."_groups d ON d.gid = h.gid 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 MONTH)) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->month;
		}else{
	...
	function leadd(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS day 
							...
							WHERE (a.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 DAY)) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
		}else{
	...
	function leadm(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_leads a 
							LEFT JOIN ".$bid."_leads_groups d ON d.gid = a.gid 
							WHERE (a.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 MONTH)) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->month;
		}else{
	...
	function mTrackerList($bid,$ofset='0',$limit='20',$type='a',$tab)
	{
		...
		$sql = "SELECT a.callid,l.lead_status FROM  ".$bid."_mtracker a
				LEFT JOIN ".$bid."_leads l ON a.leadid = l.leadid 
				LEFT JOIN ".$bid."_employee k ON k.eid = a.assignto  
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_add) ?'<li><a href="Report/mtedit/'.$r['callid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'Report/mtDelete/'.$r['callid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
			    $act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='' && file_exists($this->config->item('sound_path').$r['filename'])) ? '<li><a target="_blank" href="'.site_url('sounds/'.$r['filename']).'"><span title="Sound" class="glyphicon glyphicon-volume-up">&nbsp;Recording</span></a></li>' :'<li><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></li>') : "";
				$act .= '<li><a href="Report/followup/'.$rec['callid'].'/0/calltrack" class="btn-followup" data-toggle="modal" data-target="#modal-followup"><span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span></a></li>';		
