...
Class Groupmodel extends Model
{
	...
	function update_group($gid){
		...
		for($i=0;$i<sizeof($arr);$i++){
			...
			,"oneditaction","onhangup","replyattmsg","leadaction","oncallaction","supportaction","supportgrp","voicemessage","landingregion",
			"replytocust_missed","replytocust_attended","replytocust_repcal","replytocust_voice","replytocust_voitext"))){
	...
	function getgrouplist($bid,$ofset='0',$limit='20'){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($opt_view) ? '<li><a href="group/activerecords/'.$r['gid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-file"  title="View Group">&nbsp;View&nbsp;Group</span></a></li>':'';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'group/Delete_group/'.$r['gid'].'" class="deleteClass" title="Are you sure to Delete the group"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= ($opt_add) ? '<li><a href="AddempGroup/'.$r['gid'].'"><span class="glyphicon glyphicon-plus" title="Add Employee">&nbsp;Add&nbsp;Employee</span></a></li>':'';
				$act .= ($opt_view) ? '<li><a href="ListempGroup/'.$r['gid'].'"><span title="List Employees" class="glyphicon glyphicon-user">&nbsp;List&nbsp;Employee</span></a>':'';
				$act .= ($opt_add) ? '<li><a href="group/add_group/'.$r['gid'].'/clone"><span title="Clone Group" class="glyphicon glyphicon-th-large">&nbsp;Clone&nbsp;Group</span></a></li>':'';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= '<li><a href="'.base_url().'group/UNDelete_group/'.$r['gid'].'" class="deleteClass1" data-toggle="modal" data-target="#modal-responsive" id="'.$r['gid'].'"><img src="system/application/img/icons/undelete.png" title="Undelete" style="vertical-align:top;" />Undelete</a></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
