	 ...
	 /* MCube Track Dashboard */
		 ...
		 $.ajax({
			...
			success:function(data){
				...
				var result = jQuery.parseJSON(data);
				for(var i = 0; i < result['lastcalls'].length; i++){
					$('#lastcalls').append("<tr class='sortable coldes bd-3 bg-opacity-20  fade in ui-sortable'>     			<td>"+result['lastcalls'][i].callfrom+"</td><td>"+result['lastcalls'][i].groupname+"</td><td>"+result['lastcalls'][i].empname+"</td></tr>");
				}
				$.unblockUI();
			} 
		 });
		...
		$.ajax({
			...
			success:function(data){
				$.blockUI({ message: $('<h1><img src="system/application/img/loading.gif">Just a moment...</h1>') });
				var result = jQuery.parseJSON(data);
				$('#groupwise').append("<tr class='sortable  coldes bd-3 bg-opacity-20  fade in ui-sortable'><td><label><a href='TrackReport/all'>Weekly Total Calls</a></label></td><td align='right'>"+result['total_calls']+"</td></tr>");
	...
	/* MCube IVRS Dashboard */
	 ...
	 $.ajax({
		...
		success:function(data){
			...
			for(var i = 0; i < result['ivrs_last_calls'].length; i++){
				$('#ivrstotal').append("<tr class='sortable  coldes bd-3 bg-opacity-20  fade in ui-sortable'><td>"+result['ivrs_last_calls'][i].callfrom+"</td><td>"+result['ivrs_last_calls'][i].empname+"</td></tr>");
			}
			$.unblockUI();
		}  
	 });
	 $.ajax({
		...
		success:function(data){
			$.blockUI({ message: $('<h1><img src="system/application/img/loading.gif">Just a moment...</h1>') });
			var result = jQuery.parseJSON(data);
			$('#grwise').append("<tr class='sortable  coldes bd-3 bg-opacity-20  fade in ui-sortable'><td><label><a href='TrackReport/all'>Weekly Total Calls</a></label></td><td align='right'>"+result['ivrs_total_calls']+"</td></tr>");
	...
	/* PBX Dashboard */
	 ...
	 $.ajax({
		...
		success:function(data){
			...
			for(var i = 0; i < result['pbx_last_calls'].length; i++){
				$('#pbxtotal').append("<tr class='sortable  coldes bd-3 bg-opacity-20  fade in ui-sortable'><td>"+result['pbx_last_calls'][i].callfrom+"</td><td>"+result['pbx_last_calls'][i].groupname+"</td><td>"+result['pbx_last_calls'][i].empname+"</td></tr>");
			}
			$.unblockUI();
		}  
	 });
	 $.ajax({
		...
		success:function(data){
			$.blockUI({ message: $('<h1><img src="system/application/img/loading.gif">Just a moment...</h1>') });
			var result = jQuery.parseJSON(data);
			$('#grwise').append("<tr class='sortable  coldes bd-3 bg-opacity-20  fade in ui-sortable'><td><label><a href='TrackReport/all'>Weekly Total Calls</a></label></td><td align='right'>"+result['pbx_total_calls']+"</td></tr>");
			for(var i = 0; i < result['pbx_groupwise_todaycall'].length; i++){
				$('#pbxgrwise').append("<tr class='sortable  coldes bd-3 bg-opacity-20  fade in ui-sortable'><td>"+result['pbx_groupwise_todaycall'][i].title+"</td><td align='right'>"+result['pbx_groupwise_todaycall'][i].count+"</td></tr>");
			}
			$.unblockUI();
		}  
	 });
